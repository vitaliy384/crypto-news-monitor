...
class DeepSeekAnalyzer:
    ...
    def analyze(self, title: str, summary: str) -> Optional[Dict[str, Any]]:
        system_prompt = (
            ...
            '{"score": 7, "sentiment": "Bullish", "summary": "1 предложение — суть новости. Затем, ТОЛЬКО если есть конкретная причинно-следственная связь, второе предложение: через какой именно механизм это влияет на крипту/рынки (например: \'из-за перебоя поставок чипов вырастет дефицит майнинг-оборудования\', а не общая фраза \'что может привести к росту/падению курса\'). Если конкретного механизма нет — не пиши второе предложение вообще, шаблонные фразы без содержания хуже, чем их отсутствие."}\n'
